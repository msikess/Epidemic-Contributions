...
public class SpawnM : MonoBehaviour {
	...
	/*void Start () {
		...
		healthyPrefab.GetComponent<Transform> ().localScale = new Vector3(9f/rowCount, 9f/rowCount, 1);

		for (int j = -colCount/2; j < colCount/2; j++) {
			for (int i = -rowCount / 2; i < rowCount / 2; i++) {
				GameObject newPeep = Instantiate (healthyPrefab, new Vector3 (i*8f/rowCount + .7f, -j*5f/rowCount - .4f, 0), Quaternion.identity);
				healthy.Add (newPeep);	
	...
	public void onStart(Initializer init, GameBuilder1 world){
		...
		for (int j = -Mathf.FloorToInt(rowCount/2f); j < Mathf.CeilToInt(rowCount/2f); j++) {
			for (int i = -Mathf.CeilToInt(colCount/2f); i < Mathf.FloorToInt(colCount/2f); i++) {
				GameObject newPeep = Instantiate (healthyPrefab, new Vector3 (i*9f/colCount + .3f, -j*6f/rowCount - .9f, 0), Quaternion.identity);
				healthy.Add (newPeep);	
				newPeep.GetComponent<Rigidbody2D> ().velocity = new Vector3(Random.Range (-velocity, velocity), Random.Range (-velocity, velocity), 0);
			}
		}
